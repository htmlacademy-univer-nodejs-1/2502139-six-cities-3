...
export const postOffer = createAsyncThunk<void, NewOffer, { extra: Extra }>(
  ...
  async (newOffer, { extra }) => {
    const { api, history } = extra;
    const { data } = await api.post<Offer>(ApiRoute.Offers, {
      ...newOffer,
...
export const editOffer = createAsyncThunk<void, Omit<Offer, 'images'> & { images?: string[]}, { extra: Extra }>(
  Action.EDIT_OFFER,
  async (offer, { extra }) => {
    const { api, history } = extra;
    const newOffer = {
...
>(Action.FETCH_COMMENTS, async (id, { extra }) => {
  const { api } = extra;
  const { data } = await api.get<Comment[]>(`offers/${id}/comments`);
...
>(Action.POST_COMMENT, async ({ id, comment, rating }, { extra }) => {
  const { api } = extra;
  const { data } = await api.post<Comment[]>(`offers/${id}/comments`, {
    comment,
    rating,
  });
